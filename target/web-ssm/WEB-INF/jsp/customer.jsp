...
<body>
	...
	<div id="wrapper">
		...
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="../../login.jsp">BOOT客户管理系统 v2.0</a>
		</div>
		<!-- /.navbar-header -->
		...
		<ul class="nav navbar-top-links navbar-right">
			...
			<li class="dropdown"><a class="dropdown-toggle"
				...
				<ul class="dropdown-menu dropdown-user">
					...
					<li class="divider"></li>
					<li><a href="/web-ssm/"><i class="fa fa-sign-out fa-fw"></i>
							退出登录</a></li>
				</ul> <!-- /.dropdown-user --></li>
			<!-- /.dropdown -->
		</ul>
		...
		<div id="page-wrapper">
			...
			<div class="panel panel-default">
				<div class="panel-body">
					<form class="form-inline" action="${pageContext.request.contextPath }/resource/customer/list" method="post">
						<div class="form-group">
							<label for="customerName">客户名称</label> 
							<input type="text" class="form-control" id="customerName" value="${custName }" name="custName" data-provide="typeahead" data-items="4" autocomplete="off">
						</div>
						<div class="form-group">
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						...
						<div class="col-md-12 text-right">
							<ssmdemo:page url="${pageContext.request.contextPath }/resource/customer/list" />
						</div>
					</div>
					<!-- /.panel -->
				</div>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabelAdd">修改客户信息</h4>
				</div>
				<div class="modal-body">
					<form class="form-horizontal" id="Add_customer_form">
						<input type="hidden" id="Add_cust_id" name="cust_id" value="1"/>
						<div class="form-group">
							<label for="edit_customerName" class="col-sm-2 control-label">客户名称</label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="Add_customerName" placeholder="客户名称" name="cust_name" autocomplete="off">
							</div>
						</div>
						<div class="form-group">
							<label for="edit_customerFrom" style="float:left;padding:7px 15px 0 27px;">客户来源</label>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary" onclick="addCustomer()">添加用户</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
        $('#customerName').typeahead({
            ...
            {
                $.ajax({
                    url:"/web-ssm/resource/customer/find",
                    type:"post",
                    data:{"cust_name":query} ,
                    dataType: "json",
		...
		function editCustomer(id) {
			$.ajax({
				type:"get",
				url:"/web-ssm/resource/customer/edit",
				data:{"id":id},
                dataType: "json",
				success:function(data) {
		...
		function updateCustomer() {
			$.post("/web-ssm/resource/customer/update",$("#edit_customer_form").serialize(),function(data){
				alert("客户信息更新成功！");
        ...
        function addCustomer()
		{
            $.ajax({
                type:"post",
                url:"/web-ssm/resource/customer/add",
                ...
                success:function(data) {
                    ...
                    {alert("添加客户信息成功");
                        window.location.reload();}
                    else{alert("添加用户信息失败，请检测数据的格式要求");}
                },
		...
		function deleteCustomer(id) {
			if(confirm('确实要删除该客户吗?')) {
				$.post("/web-ssm/resource/customer/delete",{"id":id},function(data){
					alert("客户删除更新成功！");
					window.location.reload();
				});
			}
